/*  sobel-driver.c - The simplest kernel module.
...
#define DRAM_WLEN  6
#define ERROR      7
#define MEM_UNCACHED_ALLOC	 8
#define MEM_UNCACHED_RELEASE 9
...
MODULE_AUTHOR
    ("Xilinx Inc.");
MODULE_DESCRIPTION
    ("sobel-driver - loadable module template generated by petalinux-create -t modules");

#define DRIVER_NAME "sobel-driver"
...
{
    struct sobel_driver_local *lp = (struct sobel_driver_local *)file_p->private_data;
    ...
    case START:
        ...
        iowrite32(1, (void __iomem *) (lp->base_addr+START*OFFSET));
        timeout = wait_for_completion_interruptible_timeout(&lp->cmp, timeout);
        if(timeout == 0)
            printk("Timeout\n");
        iowrite32(0, (void __iomem *) (lp->base_addr+START*OFFSET));
        break;
    case IMG_SIZE:
        iowrite32(arg, (void __iomem *) (lp->base_addr+IMG_SIZE*OFFSET));
        break;
    case DRAM_RADDR:
        iowrite32(arg, (void __iomem *) (lp->base_addr+(DRAM_RADDR-1)*OFFSET));
        break;
    case DRAM_RLEN:
        iowrite32(arg, (void __iomem *) (lp->base_addr+(DRAM_RLEN-1)*OFFSET));
        break;
    case DRAM_WADDR:
        iowrite32(arg, (void __iomem *) (lp->base_addr+(DRAM_WADDR-1)*OFFSET));
        break;
    case DRAM_WLEN:
        iowrite32(arg, (void __iomem *) (lp->base_addr+(DRAM_WLEN-1)*OFFSET));
			break;
		case ERROR:
			return (long)ioread32((void __iomem *) (lp->base_addr+(ERROR-1)*OFFSET));
		case MEM_UNCACHED_ALLOC:
			lp->virt_addr = dma_alloc_coherent(lp->plat_dev_p,
				arg , &lp->phys_addr, GFP_KERNEL);
...
{
	file_p->private_data = container_of(ino->i_cdev, struct sobel_driver_local, cdev);
	return 0;
}
...
static irqreturn_t sobel_driver_irq(int irq, void *lp)
{
	struct sobel_driver_local *lpp = (struct sobel_driver_local *)lp;
	//printk("  [info]  Receive IRQ\n");
	complete(&lpp->cmp);
	//printk("sobel-driver interrupt\n");
	return IRQ_HANDLED;
}
...
// Char Device Enable
//-----------------------------------------------------------------------
static int cdevice_init(struct sobel_driver_local *lp)
{
	int rc;
...
// Char Device Disable
//-----------------------------------------------------------------------
static void cdevice_exit(struct sobel_driver_local *lp)
{
	if (lp->char_dev_p) {
...
static int sobel_driver_probe(struct platform_device *pdev)
{
	...
	lp->irq = r_irq->start;
	rc = request_irq(lp->irq, &sobel_driver_irq, IRQF_SHARED | IRQF_TRIGGER_RISING, DRIVER_NAME, lp);
	if (rc) {
	...
	dev_info(dev,"sobel-driver at 0x%08x mapped to 0x%08x, irq=%d\n",
		(unsigned int __force)lp->mem_start,
		(unsigned int __force)lp->base_addr,
		lp->irq);
