...
{
    struct test_local *lp = (struct test_local *)file_p->private_data;
    unsigned long timeout;
    volatile unsigned int *tmp = (unsigned int *)lp->base_addr;

    switch(cmd){
    case START:
        timeout = msecs_to_jiffies(10000); //Max wait : 10 second
...
{
	struct test_local *lpp = (struct test_local *)lp;
	complete(&lpp->cmp);
	// printk(KERN_INFO "IRQ\n");
	return IRQ_HANDLED;
}

static int cdevice_init(struct test_local *lp)
{
	int rc;
...
static void cdevice_exit(struct test_local *lp)
{
	if (lp->char_dev_p) {
...
static int test_probe(struct platform_device *pdev)
{
	...
	lp->irq = r_irq->start;
	rc = request_irq(lp->irq, test_irq, IRQF_SHARED | IRQF_TRIGGER_RISING, DRIVER_NAME, lp);
	if (rc) {
